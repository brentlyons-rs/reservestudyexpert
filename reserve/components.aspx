...
<script language="javascript">
    ...
    function sendTM(iTable, sCrit, sSqlField, sVal, iRow, iCol) {
	    if ((request.readyState!=4) && (request.readyState!=0)) {
            ...
            sOp = 'sendTM';
            var url="ws.asmx/SaveTM?iFirm=<%=Session["firmid"].ToString()%>&iTbl=" + iTable + "&sCrit=" + escape(sCrit) + "&sField=" + escape(sSqlField) + "&sVal=" + escape(sVal) + "&iRow=" + iRow + "&iCol=" + iCol + "&pd=y";
            gUrl = url;
    ...
    function addComponent(iRow, iCol) {
	    if ((request.readyState!=4) && (request.readyState!=0)) {
            setTimeout("sendComponent(" + iRow + ")", 3000);
        }
	    else {
            sOp = 'addComp';
            var cGeo; var cVal;
...
<form id="frmProject" method="post" runat="server" class="needs-validation">
    ...
    <table style="border-collapse: collapse; margin-left: 5px; margin-right: 5px" id="components">
        ...
        <%
            ...
            {
                sql.Clear();
                sql.Append("project_id='" + Session["projectid"].ToString() + "' and category_id=" + cboCC.Value + " and component_id=" + dr["component_id"].ToString());
            %>
        <tr>
            <td nowrap class="gridrow_wt clientHide" bgcolor=#f8f8f8 align=center nowrap><a id="aDel<%=iRow %>" href="javascript: checkDel(<%=iRow %>)" style="display: <% if (dr["year_id"].ToString() == cboYear.Value) { Response.Write("block"); } else { Response.Write("none"); } %>"><img src="images/x_white.jpg" border=0 alt="Delete" align="absmiddle"></a></td>
